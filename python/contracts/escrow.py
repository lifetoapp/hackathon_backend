...
def fa2_transfer(fa2, from_, to_, ids):
    txs_type = sp.TRecord(amount=sp.TNat, to_=sp.TAddress, token_id=sp.TNat).layout(("to_", ("token_id", "amount")))
    c = sp.contract(sp.TList(sp.TRecord(from_=sp.TAddress, txs=sp.TList(txs_type))), fa2,
...
class Marketplace(Escrow):
    ...
    def redeem_funds(self, params):
        ...
        fa2_transfer(self.data.fa2, sp.sender, params.to, params.ids)
...
class FA2(sp.Contract):
    ...
    def transfer(self, params):
        sp.set_type(params, sp.TList(sp.TRecord(from_=sp.TAddress, txs=sp.TList(
            sp.TRecord(amount=sp.TNat, to_=sp.TAddress, token_id=sp.TNat).layout(
...
def test():
    ...
    c.add_funds().run(sender=user, valid=False)
    c.add_funds().run(sender=user, amount=sp.tez(5))
    c.add_funds().run(sender=user, amount=sp.tez(1), valid=False)
    c.pull_funds().run(sender=user)
    ...
    s += c
    ids = [1, 2, 3]
    c.add_funds().run(sender=user, amount=sp.tez(1))
    c.redeem_funds(ids=ids, to=user.address, amount=sp.tez(1)).run(sender=user, valid=False)
    c.redeem_funds(ids=ids, to=user.address, amount=sp.tez(0)).run(sender=admin, valid=False)
    c.redeem_funds(ids=ids, to=user.address, amount=sp.tez(5)).run(sender=admin, valid=False)
    c.redeem_funds(ids=ids, to=user.address, amount=sp.tez(1)).run(sender=admin)
    s.verify(c.balance == sp.tez(0))
