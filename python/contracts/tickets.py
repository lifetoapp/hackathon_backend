...
def test():
    ...
    c.purchase_ticket().run(sender=user, valid=False)
    c.purchase_ticket().run(sender=user, amount=sp.tez(5))
    c.purchase_ticket().run(sender=user, amount=sp.tez(1), valid=False)
    c.refund_ticket().run(sender=user)
    ...
    s += c
    ids = [1, 2, 3]
    c.purchase_ticket().run(sender=user, amount=sp.tez(1))
    c.redeem_ticket(ids=ids, to=user.address, amount=sp.tez(1)).run(sender=user, valid=False)
    c.redeem_ticket(ids=ids, to=user.address, amount=sp.tez(0)).run(sender=admin, valid=False)
    c.redeem_ticket(ids=ids, to=user.address, amount=sp.tez(5)).run(sender=admin, valid=False)
    c.redeem_ticket(ids=ids, to=user.address, amount=sp.tez(1)).run(sender=admin)
    s.verify(c.balance == sp.tez(0))
