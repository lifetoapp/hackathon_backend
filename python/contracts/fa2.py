...
class FA2_mint(FA2_core):
    ...
    def mint(self, params):
        ...
        if self.config.store_total_supply:
            self.data.total_supply[params.token_id] = params.amount + self.data.total_supply.get(params.token_id,
                                                                                                 default_value=0)
...
def add_test(config, is_default=True):
    ...
    def test():
        ...
        scenario.p("The administrator mints 100 token-0's to User.")
        tok0_md = FA2.make_metadata(
            name="The Token Zero",
            decimals=2,
            symbol="TK0")
        c1.mint(address=user.address,
                amount=50,
                metadata=tok0_md,
        ...
        c1.balance_of(arguments_for_balance_of(consumer, [
            sp.record(owner=user.address, token_id=0),
            sp.record(owner=user.address, token_id=1),
            sp.record(owner=user.address, token_id=2)
        ]))
        ...
        if not c1.config.support_operator:
            ...
            scenario.p("User can remove operator")
            c1.update_operators([
            ...
            scenario.p("Operator1 cannot transfer User's tokens any more")
            c1.transfer(
                [
                    c1.batch_transfer.item(from_=user.address,
                                           txs=[
                                               sp.record(to_=op1.address,
                                                         amount=2,
                                                         token_id=1)])
                ]).run(sender=op1, valid=False)
            scenario.p("User can also add Operator0 for their tokens 0.")
            c1.update_operators([
                sp.variant("add_operator", c1.operator_param.make(
                    owner=user.address,
            ...
            ]).run(sender=user.address, valid=True)
